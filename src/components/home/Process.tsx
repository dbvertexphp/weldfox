...
const Process = () => {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden">
      {/* Overlay */}
      <div className="absolute top-0 left-0 w-full h-full bg-[#0033A0] opacity-90 z-0"></div>
      ...
      {/* Main Container */}
      <div
        className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-10"
      >
        {/* Header */}
        <motion.div  className="flex flex-col lg:flex-row items-start justify-between gap-6 lg:gap-12">
          {/* Left Heading */}
          <motion.div className="flex-1">
            <motion.div initial="hidden"
                      whileInView="visible"
                      viewport={{ once: true }}
                      variants={zoomInVariants} className="inline-block bg-white text-[#0033a0] text-sm sm:text-lg font-bold rounded-full px-4 py-2 mb-4">
              OUR WORK PROCESS
            </motion.div>
            <motion.h2 initial="hidden"
                      whileInView="visible"
                      viewport={{ once: true }}
                      variants={zoomInVariants} className="text-white text-2xl md:text-3xl font-bold leading-tight">
              Expert Handling from <br className="hidden sm:block" /> Start to
              Finish
            </motion.h2>
          </motion.div>
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-8">
          ...
          <motion.div initial="hidden"
                      whileInView="visible"
                      viewport={{ once: true }}
                      variants={slideUpVariants}  className="w-full lg:w-1/3 space-y-4">
            {steps.map((step, index) => (
              <div
                key={index}
                onClick={() => setSelectedStep(index + 1)}
                className={`flex flex-col items-start text-white font-semibold text-base sm:text-lg bg-[#FFA133] rounded-xl px-4 py-3 cursor-pointer transition-all duration-200 hover:scale-105 hover:z-20 hover:shadow-lg ${
          ...
          <div className="w-full lg:w-[60%]">
            <motion.div initial="hidden"
                      whileInView="visible"
                      viewport={{ once: true }}
                      variants={slideUpVariants} className="bg-white p-6 rounded-2xl shadow-lg w-full">
                ...
                <div className="w-full sm:w-2/3">
                  <div className="mb-4">
                    <motion.span initial="hidden"
                      whileInView="visible"
                      viewport={{ once: true }}
                      variants={slideUpVariants} className="text-[#3036b1] text-base sm:text-lg font-semibold">
                      STEP {selectedStep}
                    </motion.span>
